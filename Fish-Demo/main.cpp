...
fish player = { {0,0},{0,0},5,40,0,0,0 };
...
int main() {
	...
	while (!WindowShouldClose()) {
		...
		Vector2 test = { player.xy.x,player.xy.y };
		if (player.v_xy.x > 0)
		{
...
fish_NPC* create_npcfish(fishPool* pool,float x,float y ,int kind) {
		...
		{
		case 1: {
			newFish->fish.v_xy.x = (rand() % 5) - 2;
			newFish->fish.v_xy.y = (rand() % 5) - 2;
			newFish->fish.size = 20 + rand() % 30;
			newFish->fish.a = 2;
			newFish->fish.kinds = kind;
			newFish->fish.image_status = kind;
			for(int i=0;i<numberofaim;i++){
				newFish->aim[i][0] = rand() % screen_length_x;
				newFish->aim[i][1] = rand() % screen_length_y;
			}
